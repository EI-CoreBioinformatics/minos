...
class SlurmSubmitter(ABC):
	...
	def generate_resume_string(output_dir, prefix):
		...
		def resume_files_exist(prefix, r):
			return any(os.path.exists("{prefix}.resume{attempt}.{logtype}.log".format(prefix=prefix, attempt=r, logtype=logtype)) for logtype in ("out", "err"))
	
		file_prefix = "{output_dir}.{prefix}".format(output_dir=output_dir, prefix=prefix)
		r = 1
	...
	def __init__(self, cmd_args=sys.argv[1:]): 
		...
		self.partition = args.partition if hasattr(args, "partition") and args.partition else DEFAULT_PARTITION
		
		self.__compile_program_options(args, ignore_program_options=self.ignore_program_options)
	...
	def generate_submission_string(self):
		...
		self.submission_string = "{whoami}{output_dir} {program_options}".format(whoami=self.whoami, output_dir=od_string, program_options=" ".join(self.program_options))
	...
	def set_resume_string(self, prefix, program):
		if not self.require_resume_mode or (hasattr(self, "mode") and self.mode.upper() == "RESUME"):
	...
	def run(self, cleanup=True):
		...
		with cd(self.rundir if self.rundir is not None else "."):
			...
			print("Running submission command: sbatch {slurm_opts} --wrap=\"{whoami} {prog_opts}\"".format(
				whoami=self.whoami,
