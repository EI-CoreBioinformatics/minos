...
def make_exeenv_arg_group(
    parser,
    default_hpc_config_file=DEFAULT_HPC_CONFIG_FILE,
    ...
    hpc_group = parser.add_argument_group(
        "HPC Options", "Controls for how jobs should behave across the HPC resources."
    )
    if allow_mode_selection:
        hpc_group.add_argument(
            "--mode",
            choices=[rm.name.lower() for rm in RunMode],
            default="normal",
            help=dedent(
                """This option controls how to run the pipelines.
...
class ExecutionEnvironment:
    ...
    def __init__(self, args=None, now="eicore", job_suffix=None, log_dir="logs"):
        ...
        if args:
            scheduler = (
                args.scheduler if args.scheduler and args.scheduler != "" else "SLURM"
            )
            ...
            job_name = (
                "{rule}_" + job_suffix if job_suffix and job_suffix != "" else "{rule}"
            )
            if scheduler.upper() == "LSF":
                self.sub_cmd = "bsub"
                self.res_cmd = (
                    " -R rusage[mem={cluster.memory}]span[ptile={threads}] -n {threads} -q "
                    + self.partition
                    + " -J "
                    + job_name
                    + " -oo "
                ...
                self.sub_cmd = "qsub"
                self.res_cmd = (
                    " -lselect=1:mem={cluster.memory}MB:ncpus={threads} -q "
                    + self.partition
                    + " -N "
                    + job_name
                    + " -o "
    ...
    if dryrun:
        ...
        if res:
            dag_file = os.path.join(
                out_dir,
                "eicore-"
                + os.path.basename(snakefile).split(".")[0]
                + "_dag-"
                + NOW
                + ".dot",
            )
        ...
        cluster = (
            (exe_env.sub_cmd + exe_env.res_cmd if not exe_env.use_drmaa else None)
            if exe_env.use_scheduler
            else None
        )
        ...
        res = snakemake(
            ...
            printreason=True,
            stats=os.path.join(
                out_dir, os.path.basename(snakefile) + "-" + NOW + ".stats"
            ),
