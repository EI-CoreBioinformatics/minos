...
STATS_ORDER = [
	"Number of genes", "Number of Transcripts", "Transcripts per gene", "Number of monoexonic genes", "Monoexonic transcripts", "Transcript mean size cDNA (bp)", "Transcript median size cDNA (bp)", "Min cDNA", "Max cDNA", 
	"Total exons", "Exons per transcript", "Exon mean size (bp)", "CDS mean size (bp)", "Transcript mean size CDS (bp)", "Transcript median size CDS (bp)", "Min CDS", "Max CDS", "Intron mean size (bp)", "5'UTR mean size (bp)", 
	"3'UTR mean size (bp)"
]
...
def main():
	...
	for row in csv.DictReader(open(args.stats_tsv), delimiter="\t"):
		...
		if stat.lower() in {
			...
			stats_info["{} mean size (bp)".format(stat.split(" ")[0])] = format_number(row["Average"])

		elif stat.lower() in {"cdna lengths", "cds lengths"}:
			stats_info["Transcript mean size {} (bp)".format(stat.split(" ")[0])] = format_number(row["Average"])
			stats_info["Transcript median size {} (bp)".format(stat.split(" ")[0])] = format_number(row["Median"])
