        ...
        while walk:
            ...
            if cwd_base == "CPC-2.0_beta":
                ...
                for mid in dirs:
                    ...
                    rows.append(
                        (
                            ExternalMetrics.MIKADO_TRANSCRIPTS_OR_PROTEINS,
                            mclass,
                            mid,
                            path,
                        )
                ...
                for mid in dirs:
                    ...
                    pattern = (
                        r"\DDIAMOND\D*\Dtophit"
                        if use_diamond
                        else r"\DBLAST\D*\Dtophit"
                    )
                    for f in filter(lambda x: re.search(pattern, x), files):
                        path = os.path.join(cwd, f)
                        rows.append(
                            (ExternalMetrics.PROTEIN_BLAST_TOPHITS, mclass, mid, path)
                        )
                ...
                for mid in dirs:
                    ...
                    rows.append(
                        (ExternalMetrics.KALLISTO_TPM_EXPRESSION, mclass, mid, path)
                    )
            elif cwd_base == "repeats":
                mclass = "repeat"
                for path in glob.glob(
                    os.path.join(cwd, "*.no_strand.exon.gff.cbed.parsed.txt")
                ):
                ...
                mid = busco_data
                path = os.path.join(
                    metrics_path, "busco_proteins", "busco_proteins.tsv"
                )
