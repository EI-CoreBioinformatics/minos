        ...
        for line in self.args.fasta:
            ...
            if line.startswith(">"):
                ...
                if self.args.add_fields:
                    cols = (y.strip() for y in self.args.add_fields.split(","))
                    header = (
                        header
                        + " "
                        + " ".join(
                ...
                if self.args.add_columns:
                    cols = (
                        int(y.strip()) - 1 for y in self.args.add_columns.split(",")
                    )
...
def main():
    parser = argparse.ArgumentParser(description="Script to clean fasta header")
    parser.add_argument(
        "fasta",
        nargs="?",
        type=argparse.FileType("r"),
        default=sys.stdin,
        help="Provide fasta file (as a file or stdin)",
    )
    parser.add_argument(
        "--add_fields",
        nargs="?",
