...
def read_metrics_info(f):
    ...
    for row in csv.reader(open(f), delimiter="\t"):
        ...
        metrics_info.setdefault(metrics_type, collections.OrderedDict())[row[1]] = row[
            2
        ]
...
def generate_metrics_header(metrics_info):
    ...
    for mcid in MCLASS_INFO:
        for mid in metrics_info.get(mcid, collections.OrderedDict()):
            header.extend(
                [mid + (("_" + m) if m else m) for m in MCLASS_INFO[mcid]["metrics"]]
            )
...
def read_mikado_refmap(f, seen=set()):
    ...
    for i, row in enumerate(csv.reader(open(f), delimiter="\t")):
        if i > 0 and not row[0].startswith("#"):
            ...
            if seen and tid not in seen:
                raise ValueError(
                    "Error: This is not the first processed mikado run ({}) but transcript {} occurs for the first time. Please check your mikado inputs.".format(
                        f, tid
                    )
                )
            ...
            model_info[tid] = collections.OrderedDict(
                zip(
                    ("nF1", "jF1", "eF1", "aF1"),
                    map(_round_perc_frac, (nF1, jF1, eF1, aF1)),
                )
            )
    ...
    if seen:
        ...
        if delta:
            raise ValueError(
                "Error: Mikado run ({}) has less transcripts than previously processed run ({} missing transcript(s): {}). Please check your mikado inputs.".format(
                    f, len(delta), ",".join(sorted(delta)[:3]) + ", ..."
                )
            )
...
def read_blast(f, seen=set()):
    ...
    for i, row in enumerate(csv.reader(open(f), delimiter="\t")):
        if not row[0].startswith("#"):
            ...
            if seen and tid not in seen:
                raise ValueError(
                    "Error: This is a blast run ({}) but transcript {} occurs for the first time. Please check your mikado/blast inputs.".format(
                        f, tid
                    )
                )
            ...
            model_info[tid] = collections.OrderedDict(
                zip(("qCov", "tCov"), map(_round_perc_frac, (qCov, tCov)))
            )
...
def read_cpc(f, seen=set()):
    ...
    for i, row in enumerate(csv.reader(open(f), delimiter="\t")):
        if i > 0 and not row[0].startswith("#"):
            ...
            if seen and tid not in seen:
                raise ValueError(
                    "Error: This is a cpc run ({}) but transcript {} occurs for the first time. Please check your mikado/cpc inputs.".format(
                        f, tid
                    )
                )

            model_info[tid] = collections.OrderedDict(
                {"cpc": _round_frac(clean_na(row[6]))}
            )
...
def read_kallisto(f, seen=set()):
    ...
    for i, row in enumerate(csv.reader(open(f), delimiter="\t")):
        if i > 0 and not row[0].startswith("#"):
            ...
            if seen and tid not in seen:
                raise ValueError(
                    "Error: This is a kallisto run ({}) but transcript {} occurs for the first time. Please check your mikado/kallisto inputs.".format(
                        f, tid
                    )
                )

            model_info[tid] = collections.OrderedDict(
                {"tpm": _round_frac(clean_na(row[4]))}
            )
...
def read_rm_repeats(f, seen=set()):
    ...
    for i, row in enumerate(csv.reader(open(f), delimiter="\t")):
        if row and not row[0].startswith("#"):
            ...
            if seen and tid not in seen:
                raise ValueError(
                    "Error: This is a repeat modeler run ({}) but transcript {} occurs for the first time. Please check your mikado/rm inputs.".format(
                        f, tid
                    )
                )

            model_info[tid] = collections.OrderedDict(
                {"cov": _round_frac(clean_na(row[3]))}
            )
    ...
    for i, row in enumerate(csv.reader(open(f), delimiter="\t")):
        if row and not row[0].startswith("#"):
            if row[1] != "Missing":
                ...
                if seen and tid not in seen:
                    raise ValueError(
                        "Error: This is a busco run ({}) but transcript {} occurs for the first time. Please check your mikado/rm inputs.".format(
                            f, tid
                        )
                    )
            ...
            for run in model_info:
                try:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Error: Missing {} values for transcript {}".format(
                                run, tid
                            )
                        )
                row.extend(data)
