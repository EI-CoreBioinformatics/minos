    ...
    with open(_out, "w") as blast_out:
        ...
        for row in csv.reader(open(_in), delimiter="\t"):
            if row and not row[0].startswith("#") and not row[0] in seen:
                if len(row) < 17:
                    raise ValueError(
                        "Misformatted blastp line (ncols={ncols}) in {blastp_input}. Expected is outfmt 6 (17 cols).".format(
                            ncols=len(row), blastp_input=_in
                        )
                    )
                try:
                    ...
                    raise ValueError(
                        "Misformatted blastp line: could not parse integer values from columns 2-7 ({})".format(
                            ",".join(row[3:9])
                        )
                    )
                qcov, scov = (
                    calc_coverage_perc(qstart, qend, qlen),
                    calc_coverage_perc(sstart, send, slen),
                )
