...
def get_busco_categories(max_copy_number=4):
    return [
        "Complete_{}{}".format(n, "+" if n == max_copy_number else "")
        for n in range(1, max_copy_number + 1)
    ] + MAIN_CATEGORIES
...
def read_full_table(table_file, tx2gene=None, is_pick=False, max_copy_number=4):
    ...
    counts = Counter(
        {cat: 0 for cat in get_busco_categories(max_copy_number=max_copy_number)}
    )
    ...
    for row in csv.reader(open(table_file), delimiter="\t"):
        ...
        if row[1] in {"Missing", "Fragmented"}:
            ...
            complete.setdefault(row[0], list()).append(
                (row[2], get_gene_id(row[2], tx2gene=tx2gene, is_pick=is_pick))
            )
    ...
    for cat, genes in complete.items():
        if tx2gene is not None or is_pick:
            ...
            n = len(genes)
        cat = "Complete_{}{}".format(
            min(n, max_copy_number), "+" if n >= max_copy_number else ""
        )
        counts[cat] += 1
