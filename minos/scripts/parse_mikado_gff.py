...
class GffRecord():
	__fields = ["seqid", "source", "type", "start", "end", "score", "strand", "phase", "attributes"]
	...
	def __init__(self, data):
		try:
			...
			raise ValueError("Error when parsing gff line:\n{}\n".format("\t".join(data)))
	...
	def get_attribute(self, attr_id):
		try:
			...
			raise KeyError("Error: Could not find attribute {} in\n{}\n".format(attr_id, str(self)))
	...
	def normalize_feature(self):
		...
		try:
			...
			raise ValueError("Error: Invalid feature {} type in \n{}\n".format(self.type, str(self)))
			...
			if row[0]:
				if row[0].startswith("#"):
					...
					if gff_rec.is_feature("superlocus"):
						...
						gff_rec.normalize_feature()
					elif any(gff_rec.is_feature(feat) for feat in ("mRNA", "ncRNA", "transcript")):
						gff_rec.normalize_feature()
						...
						new_attrib["Note"].extend("{}:{}".format(k, v[0]) for k, v in gff_rec.attributes.items() if k not in preserve_attribs and k.lower() != "note")
						new_attrib["Note"].extend(gff_rec.attributes.get("note", [""])[0].split("|")[1:])
						new_attrib["Note"] = ",".join(new_attrib["Note"])
						gff_rec.attributes = ";".join("{}={}".format(k, v) for k, v in new_attrib.items())
