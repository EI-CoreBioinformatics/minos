...
import glob
import csv
from minos.scripts.analyse_busco import (
    read_full_table,
    read_tx2gene,
    get_busco_categories,
)
...
class BuscoTableGenerator:
    ...
    def import_txcoords(self, txcoords_prepare, txcoords_pick):
        self.txcoords_prepare = dict(
            line.strip().split("\t") for line in open(txcoords_prepare)
        )
        self.txcoords_pick = dict(
            line.strip().split("\t") for line in open(txcoords_pick)
        )
    ...
    def parse_rundata(self, busco_run_path):
        ...
        for d in os.listdir(busco_run_path):
            if d.endswith("_final") or d == "genome":
                ...
                (
                    ...
                    _,
                    fragmented_buscos,
                ) = read_full_table(f, is_pick=d.endswith("_final"))
                if d == "proteins_final":
                ...
                for dd in glob.glob(os.path.join(busco_run_path, d, "*")):
                    if os.path.basename(dd) != "input":
                        ...
                        (
                            self.run_tables[runid],
                            complete_buscos,
                            missing_buscos,
                            fragmented_buscos,
                        ) = read_full_table(f, self.tx2gene)
        ...
        if len(missing_busco_proteins_list) > 0:
            self.missing_busco_proteins.update(
                missing_busco_proteins_list[0].intersection(
                    *missing_busco_proteins_list
                )
            )
        if len(missing_busco_transcripts_list) > 0:
            self.missing_busco_transcripts.update(
                missing_busco_transcripts_list[0].intersection(
                    *missing_busco_transcripts_list
                )
            )

    def __init__(
        self, tx2gene_data_dir, txcoords_prepare, txcoords_pick, busco_run_path
    ):
    ...
    def write_review_table(self, prefix):
        ...
        with open(prefix + ".review_table", "w") as review_out:
            print(
                *BuscoTableGenerator.REVIEW_TABLE_HEADER,
                sep="\t",
                flush=True,
                file=review_out
            )
            ...
            for bid in sorted(review_proteins):
                tid = ",".join(
                    self.fragmented_busco_proteins["proteins_final"].get(bid, list())
                )
                ...
                for protein_set in self.complete_busco_proteins.values():
                    prepare_tids.extend(
                        item[0] for item in protein_set.get(bid, list())
                    )
                prepare_coords = [
                    self.txcoords_prepare.get(ptid, None) for ptid in prepare_tids
                ]
                row = [
                    bid,
                    tid,
    ...
    def write_busco_table(self, prefix, max_copy_number):
        with open(prefix, "w") as table_out:
            ...
            for cat in get_busco_categories(max_copy_number=max_copy_number):
                ...
                if cat.startswith("Complete_"):
                    ...
                    cat_lbl = (
                        ...
                        if copies == "1"
                        else "Complete ({} copies)".format(copies)
                    )

                print(
                    cat_lbl,
                    *(v[cat] for v in self.run_tables.values()),
                    sep="\t",
                    flush=True,
                    file=table_out
                )
            ...
            print(
                "# best achievable protein BUSCO count: {}".format(
                    len(complete_busco_proteins_)
                ),
                flush=True,
                file=table_out,
            )
            print(
                "# best achievable transcript BUSCO count: {}".format(
                    len(complete_busco_transcripts_)
                ),
                flush=True,
                file=table_out,
            )
            print(
                "# lowest achievable missing protein BUSCO count: {}".format(
                    len(self.missing_busco_proteins)
                ),
                flush=True,
                file=table_out,
            )
            print(
                "# lowest achievable missing transcript BUSCO count: {}".format(
                    len(self.missing_busco_transcripts)
                ),
                flush=True,
                file=table_out,
            )
