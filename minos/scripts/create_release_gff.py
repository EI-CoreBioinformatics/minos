...
class GffReleaseGenerator:
    ...
    def __read_metrics_info(self, metrics_tsv):
        ...
        for row in csv.DictReader(open(metrics_tsv), delimiter="\t"):
            ...
            if self.transcript_metrics_info.get(tid, None) is not None:
                raise ValueError(
                    "Error: Transcript '{}' has already been processed - caused by potential duplicate entry.".format(
                        tid
                    )
                )
            ...
            if self.gene_metrics_info.get(gid, None) is None:
                self.gene_metrics_info[gid] = {col: row[col] for col in gene_cols}
    ...
    def process_gff(
        self, raw_gff, name_prefix="XYZ_v1", file_prefix="mikado.annotation"
    ):
        ...
        out_release = os.path.join(
            os.path.dirname(raw_gff), file_prefix + ".release.unsorted.gff3"
        )
        out_browser = os.path.join(
            os.path.dirname(raw_gff), file_prefix + ".release_browser.unsorted.gff3"
        )

        with open(raw_gff + ".old_new_id_relation.txt", "wt") as logfile, open(
            out_release, "wt"
        ) as out_rel, open(out_browser, "wt") as out_brw:
            print(
                *["#New_Gene_ID", "New_mRNA_ID", "Old_Gene_ID", "Old_mRNA_ID"],
                sep="\t",
                file=logfile
            )
            ...
            try:
                for row in csv.DictReader(
                    open(raw_gff), delimiter="\t", fieldnames=self.gff_header
                ):
                    if row["seqid"].startswith("#"):
                        print(
                            *(c for c in row.values() if c is not None),
                            sep="\t",
                            file=out_rel
                        )
                        print(
                        ...
                        if row["type"].lower() in {"gene", "ncrna_gene"}:
                            ...
                            if ginfo is None:
                                raise ValueError(
                                    "Error: Gene '{}' is not in the metrics file.\n{}\n".format(
                                        gid, "\t".join(row.values())
                                    )
                                )
                            ...
                            if eval(ginfo["discard"]):
                                print(
                                    "Debug: Gene discarded '{}'".format(gid),
                                    file=sys.stderr,
                                )
                                continue
                            ...
                            if new_genes.get(gid, None) is not None:
                                raise ValueError(
                                    "Error: Duplicated gene id enocuntered in the input gff '{}'".format(
                                        gid
                                    )
                                )
                            ...
                            row[
                                "attributes"
                            ] = "ID={0};Name={0};biotype={1};confidence={2}".format(
                                new_gene_id, ginfo["biotype"], ginfo["confidence"]
                            )
                            ...
                            note = dict(
                                item.split(":") for item in note if item.count(":") == 1
                            )
                            is_primary = note.get("primary") is not None and eval(
                                note.get("primary")
                            )
                            ...
                            if tinfo is None:
                                raise ValueError(
                                    "Error: Transcript '{}' is not in the metrics file.\n{}\n".format(
                                        tid, "\t".join(row.values())
                                    )
                                )
                            ...
                            if eval(tinfo["discard"]):
                                print(
                                    "Debug: Transcript discarded '{}'".format(tid),
                                    file=sys.stderr,
                                )
                                continue
                            ...
                            try:
                                ...
                                raise ValueError(
                                    "Error: Cannot extract suffix count from transcript id ({}).".format(
                                        tid
                                    )
                                )
                            ...
                            if new_gene is None:
                                raise ValueError(
                                    "Error: No gene feature for transcript id {} (gene={}).".format(
                                        tid, gid
                                    )
                                )

                            new_transcript = "{}.{}".format(
                                new_gene, primary_suffix_check
                            )
                            if new_transcripts.get(tid, None) is not None:
                                raise ValueError(
                                    "Error: Duplicated transcript id '{}' ({})".format(
                                        tid, new_transcript
                                    )
                                )
                            ...
                            row["attributes"] = (
                                attribs
                                + ";confidence={};representative={}".format(
                                    tinfo["confidence"], is_primary
                                )
                            )
                            print(*row.values(), sep="\t", file=out_rel)
                            ...
                            if eval(tinfo["discard"]):
                                print(
                                    "Debug: {} parent transcript discarded '{}'".format(
                                        row["type"], tid
                                    ),
                                    file=sys.stderr,
                                )
                            ...
                            if new_transcript is not None:
                                new_feature_id = "{}.{}{}".format(
                                    new_transcript,
                                    row["type"],
                                    feature_counter[row["type"]],
                                )
                                row["attributes"] = "ID={};Parent={}".format(
                                    new_feature_id, new_transcript
                                )
                                print(*row.values(), sep="\t", file=out_rel)
                                print(*row.values(), sep="\t", file=out_brw)
                ...
                raise FileNotFoundError(
                    "Error: Could not find gff file at {}".format(raw_gff)
                )
