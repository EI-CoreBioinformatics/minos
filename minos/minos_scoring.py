...
EXTERNAL_METRICS_HEADERS = [
    "metric_name_prefix",
    "metric_class",
    "multiplier",
    "not_fragmentary_min_value",
    "file_path",
]
...
class ScoringMetricsManager(object):
    ...
    def __importMetricsData(self, fn, use_tpm=False):
        ...
        for row in csv.reader(open(fn), delimiter="\t", quotechar='"'):
            try:
                ...
                raise ValueError(
                    "External metrics record has to comply to the format: {}\n{}".format(
                        EXTERNAL_METRICS_HEADERS, row
                    )
                )
            ...
            if "." in metric_name:
                raise ValueError(
                    "Metric name {} contains invalid character '.'. Please adjust metric names accordingly.".format(
                        metric_name
                    )
                )
            files = files.split(",")
            for f in files:
                if not os.path.exists(f):
                    raise ValueError(
                        "Missing input file for metric {}: {}".format(metric_name, f)
                    )

            data = {"multiplier": multiplier, "min_value": nf_minval, "files": files}

            if metric_class == "expression":
                if use_tpm:
                    if metric_name[-3:] not in ScoringMetricsManager.STRANDINFO:
                        raise ValueError(
                            "Expression metric {} does not have strandedness information. Please add a suffix to indicate strandedness ({})".format(
                                metric_name, ScoringMetricsManager.STRANDINFO
                            )
                        )
                else:
                    print(
                        "Found expression metric: {} but --use-tpm-for-picking was not set. Commenting it on scoring file.".format(
                            metric_name
                        )
                    )

            self.metrics.setdefault(metric_class, OrderedDict()).setdefault(
                metric_name, list()
            ).append(data)

        if len(self.metrics.get("junction", OrderedDict())) > 1:
            raise ValueError(
                "More than one junction file supplied. "
                + self.metrics.get("junction", list())
            )
    ...
    def getMetricsData(self, metric):
        ...
        for k in self.metrics.get(metric, OrderedDict()):
            mdata.setdefault(k, list()).extend(
                item["files"] for item in self.metrics[metric][k]
            )
            pass
        return mdata

    def generateScoringFile(
        self, scoring_template, outfile, busco_scoring=None, use_tpm=False
    ):
            ...
            for mclass, runs in metrics.items():
                if mclass not in NON_NF:
                    ...
                    for runid, run in runs.items():
                        for suffix in suffixes:
                            params.append(
                                "    external.{}_{}: {{operator: gt, value: {}}}".format(
                                    runid, suffix, run[0]["min_value"]
                                )
                            )
        ...
        def parse_multiplier(m, k=4):
            if "," in m:
                ...
                if len(m_split) != k:
                    raise ValueError(
                        "multi-multiplier string ({}) does not contain expected multipliers (k={})".format(
                            m, k
                        )
                    )
        ...
        def generate_external_scoring(metrics):
            ...
            for mclass, runs in metrics.items():
                ...
                for runid, run in runs.items():
                    ...
                    multipliers = parse_multiplier(
                        run[0]["multiplier"], k=len(suffixes)
                    )
                    for suffix in suffixes:
                        ...
                        if multiplier is None:
                            ...
                            if multiplier is None:
                                raise ValueError(
                                    "Wasn't able to parse multiplier from {}".format(
                                        run[0]["multiplier"]
                                    )
                                )
                        ...
                        if suffix == "tpm":
                            expression = "{{rescaling: max, multiplier: {}}}".format(
                                multiplier
                            )
                            suffix = ""
                            if not use_tpm:
                            ...
                            expression = "{{rescaling: max, use_raw: true, multiplier: {}}}".format(
                                multiplier
                            )
                            suffix = "_" + suffix
                        scoring.append(
                            "  {}external.{}{}: {}".format(
                                comment, runid, suffix, expression
                            )
                        )
        ...
        if busco_scoring is None:
            ...
            if bs_multiplier < 0:
                raise ValueError(
                    "Invalid value for busco scoring multiplier: {}".format(
                        bs_multiplier
                    )
                )
        ...
        with open(scoring_template) as _in, open(outfile, "wt") as _out:
            for line in _in:
                if line.strip().startswith("### MINOS:GENERATE_NF_EXPRESSION"):
                    line = next(_in)
                    print(
                        "  expression:", generate_nf_expression(self.metrics), file=_out
                    )
                    ...
                    print(
                        "  {comment}external.busco_proteins: {{rescaling: max, use_raw: true, multiplier: {multiplier}}}".format(
    ...
    def get_scoring_data(self):
        return {
            label: self.getMetricsData(key) for key, label in METRIC_KEY_LABELS.items()
        }
