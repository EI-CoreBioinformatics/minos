...
class BuscoConfiguration(dict):
    ...
    def parse_busco_levels(levels):
        ...
        if invalid_levels:
            raise ValueError(
                "Invalid busco levels specified with --busco-level option ({}). Valid levels are {}.".format(
                    invalid_levels, BUSCO_LEVELS
                )
            )
    ...
    def _check_precomputed_genome_run(self, bg_run):
        ...
        busco_data = {
            ...
            "missing_busco_list": {
                "fn": "missing_busco_list.tsv",
                "label": "missing busco list",
            },
        }
        if bg_run is not None:
            ...
            for key, vals in busco_data.items():
                try:
                    precomputed_genome[key] = glob.glob(
                        os.path.join(bg_run, vals["fn"])
                    )[0]
                except:
                    raise ValueError(
                        "No valid {label} file in {bg_run}".format(
                            label=vals["label"], bg_run=bg_run
                        )
                    )
    ...
    def __init__(self, args):
        (
            ...
            self["transcriptome"],
            self["genome"],
        ) = BuscoConfiguration.parse_busco_levels(args.busco_level)
        self["proteins"] = self["proteins"] or args.busco_scoring is not None
        if self["genome"] and args.busco_genome_run:
            raise ValueError(
                "BUSCO genome was selected ({}) together with --busco_genome_run ({}).".format(
                    args.busco_level, args.busco_genome_run
                )
            )
        if any(self.values()) and (
            args.busco_lineage is None or not os.path.exists(args.busco_lineage)
        ):
            raise ValueError(
                "BUSCO analysis requested (P:{}, T:{}, G:{}) but no valid lineage specified ({}).".format(
                    *self.values(), args.busco_lineage
                )
            )
        if args.busco_lineage and not set(args.busco_level.split(",")).issubset(
            BUSCO_LEVELS.difference({"off", "none"})
        ):
            raise ValueError(
                "BUSCO lineage (--busco-lineage ({})) requires --busco-level ({} - user provided) not in {{off,none}})".format(
                    args.busco_lineage, args.busco_level
                )
            )

        self["precomputed_genome"] = self._check_precomputed_genome_run(
            args.busco_genome_run
        )
