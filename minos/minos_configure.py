...
class MinosRunConfiguration(dict):
    def _run_mikado_configure(self, args):
        cmd = MIKADO_CONFIGURE_CMD.format(
            ...
            codon_table=str(args.codon_table - 1)
            if args.codon_table == 1
            else args.codon_table,
            list_file=args.list_file,
            external_metrics=(" --external " + args.external + " ")
            if args.external
            else " ",
            ...
            junctions=(
                " --junctions "
                + list(self.smm.getMetricsData("junction").values())[0][0][0]
                + " "
            )
            if self.smm.getMetricsData("junction")
            else " ",
    ...
    def _generate_scoring_file(self, args):
        ...
        print(
            "Generating scoring file " + self.scoring_file + " ...", end="", flush=True
        )
        self.smm.generateScoringFile(
            args.scoring_template,
            self.scoring_file,
            busco_scoring=args.busco_scoring,
            use_tpm=args.use_tpm_for_picking,
        )
        print(" done.")
    ...
    def __init__(self, args):
        ...
        self.mikado_config_file = os.path.join(
            args.outdir, args.prefix + ".mikado_config.yaml"
        )
    ...
    def _generate_run_configuration(self, args):
        ...
        self["data"] = {
            "transcript_models": {
                row[1]: row[0]
                for row in csv.reader(open(args.list_file), delimiter="\t")
            }
        }
        ...
        with open(
            os.path.join(args.outdir, args.prefix + ".run_config.yaml"), "wt"
        ) as run_config_out:
            yaml.dump(
                dict(self.items()),
                run_config_out,
